...
src/
  api/
    ...
    users/
      users.ts
    cart/
      cart.ts           ← typy + funkcje API dla koszyków (admin)
    auth/
      auth.ts
  ...
  features/             ← moduły funkcjonalne
    ...
    cart/
      components/
        CartItemRow.tsx
        CartStatsCard.tsx
      hooks/            ← useAdminCarts, useCartStats, useAdminUserCart, useAdminUpdateCartItem
      pages/
        CartsListPage.tsx
        CartStatsPage.tsx
        UserCartPage.tsx
...
## Konwencje — moduł Cart (admin)

### Pliki API (`src/api/cart/cart.ts`)
...
export const getCarts = (params: { page: number; pageSize: number }) =>
  apiClient
    .get<PagedResult<CartListItem>>("/admin/cart", { params })
    .then((r) => r.data);

export const getCartStats = () =>
  apiClient.get<CartStats>("/admin/cart/stats").then((r) => r.data);

export const getUserCart = (userId: string) =>
  apiClient.get<Cart>(`/admin/cart/${userId}`).then((r) => r.data);

export const adminUpdateCartItem = (
  userId: string,
  itemId: string,
  quantity: number,
) => apiClient.put(`/admin/cart/${userId}/items/${itemId}`, { quantity });

export const adminRemoveCartItem = (userId: string, itemId: string) =>
  apiClient.delete(`/admin/cart/${userId}/items/${itemId}`);

export const adminClearCart = (userId: string) =>
  apiClient.delete(`/admin/cart/${userId}`);
```

### Query keys

```typescript
["admin", "carts"][("admin", "carts", "stats")][("admin", "carts", userId)]; // lista koszyków // statystyki // koszyk konkretnego użytkownika
```

### Uwagi UX

- **`expiresAt`**: pokaż jako względny czas (np. "wygasa za 3 dni") i podświetl na czerwono jeśli < 24h
- **`currentPrice` w itemach**: live z katalogu — może różnić się od ceny w chwili dodania (brak `priceAtAdded` w backendzie)
- **Statystyki**: `abandonedCartsCount` = koszyki aktywne bez aktywności > 1 dzień (logika backendowa, admin tylko wyświetla)
- **`topProducts`**: lista posortowana malejąco wg `totalQuantity`
